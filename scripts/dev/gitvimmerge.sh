#!/bin/bash
# A better vimdiff mergetool for Git (https://github.com/whiteinge/dotfiles/blob/master/bin/diffconflicts)
#
...
cat $(realpath $0) > /tmp/MERGE-HELP

$cmd -f -R -d "${LCONFL}" "${RCONFL}" \
    -c ":set noro | winc l | set noma | winc h" \
    -c ":tabe $QLOCAL | vert diffs $QBASE | vert diffs $QREMOTE" -c ":windo set nowrap | windo set noma" \
    -c ":winc t" -c ":tabe $QMERGED" \
    -c ":tabe /tmp/MERGE-HELP" \
    -c ":tabfir" 
